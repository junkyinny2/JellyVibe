...
sub onButtonSelectedChange()
    ...
    selectedButton.blendColor = chainLookupReturn(m.global.session, "user.settings.colorCursor", "#7B2FBE")
end sub
    ...
    if press
        ...
        if m.song.hasFocus()
            if key = "down"
                ...
                m.artist.color = chainLookupReturn(m.global.session, "user.settings.colorCursor", "#7B2FBE")
                return true
            end if
        ...
        if m.artist.hasFocus()
            ...
            if key = "up"
                ...
                m.song.color = chainLookupReturn(m.global.session, "user.settings.colorCursor", "#7B2FBE")
                return true
            end if
            if key = "OK" or key = "play"
                currentItem = m.global.queueManager.callFunc("getCurrentItem")
