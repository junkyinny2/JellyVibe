...
sub onItemFocused()
    ...
    for each radioOption in m.radioOptions.getChildren(-1, 0)
        ...
        if isValid(itemText)
            if itemText.isSubType("ScrollingText")
                itemText.color = chainLookupReturn(m.global.session, "user.settings.colorDialogText", "#ffffff")
            end if
        end if
    end for
    ...
    if isValid(itemText)
        if itemText.isSubType("ScrollingText")
            itemText.color = chainLookupReturn(m.global.session, "user.settings.colorDialogSelectedText", "#ffffff")
        end if
    end if
    moveScrollBar()
...
'//# sourceMappingURL=./RadioDialog.brs.map