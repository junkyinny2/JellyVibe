...
sub onFocusChange()
    if m.top.focus
        ...
        group.lastFocus = m.top.getChild(m.top.displayedIndex)
    end if
end sub

sub displayedIndexChanged()
    if not m.top.affectsFocus then
        return
    end if
    ...
    group = m.global.sceneManager.callFunc("getActiveScene")
    group.lastFocus = displayedSection
...
'//# sourceMappingURL=./sectionScroller.brs.map