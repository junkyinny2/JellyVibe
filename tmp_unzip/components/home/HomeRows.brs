...
sub init()
    ' WARNING: All items use a 12px padding architecture for focus highlights.
    ' m.top.width/height is the CELL size (Poster + 12px padding).
    ' Child elements should be sized at (m.top.width - 12).
    ' See UI_ARCHITECTURE.md for details.
...
sub updateSize()
    ' Translation now handled in Home.xml for better visibility
    itemHeight = 342 ' 330 + 12 for highlight padding
    ...
    m.top.itemSpacing = [
        0
        93
    ] ' 105 - 12 to maintain visual gap
    ' spacing between items in a row
    m.top.rowItemSpacing = [
        8
        0
    ] ' 20 - 12 to maintain visual gap
...
sub processUserSections()
    ...
    if m.top.viewMode = "favorites"
        m.expectedRowCount = 1
    ...
    for i = 0 to 7
        ...
        sectionName = bslib_coalesce(userSection, "none")
...
function getOriginalSectionIndex(sectionName as string) as integer
    ...
    for i = 0 to 7
        userSection = sessionUser.settings["homesection" + i.toStr()]
        settingSectionName = bslib_coalesce(userSection, "none")
...
sub setRowItemSize()
    ...
    for i = 0 to homeSections.count() - 1
        if isValid(homeSections[i].cursorSize) then
            baseSize = homeSections[i].cursorSize
        else
            baseSize = ([
                464
                261
...
sub createLibraryRow()
    ...
    ' IMPORTANT: Work on a shallow copy of the array to avoid corrupting shared m.libraryData
    libraries = filterNodeArray(m.libraryData, "id", m.global.session.user.configuration.MyMediaExcludes)
    filteredMedia = []
    filteredMedia.Append(libraries)
    ' Reorder filteredMedia to prioritize Movies, TV Shows, and Collections
    reorderedMedia = []
    priorityTypes = [
        "movies"
        "tvshows"
    ...
    ' 1. Pull prioritized items in exact order
    for each pType in priorityTypes
        for i = 0 to filteredMedia.count() - 1
            item = filteredMedia[i]
            if item <> invalid
                itemCollectionType = LCase((function(item)
                        __bsConsequent = item.collectionType
                ...
                if itemCollectionType = pType
                    reorderedMedia.push(item)
                    filteredMedia[i] = invalid ' Safe to invalidate now as we are on a copy
                    exit for
                end if
            end if
        end for
    end for
    ' 2. Add the rest of the libraries
    for each item in filteredMedia
        if item <> invalid
            reorderedMedia.push(item)
        end if
    end for
    for each item in reorderedMedia
        ' Skip items that have no ID or Name, which can appear as "empty boxes"
        if isValidAndNotEmpty(item.id) and isValidAndNotEmpty(item.name)
...
sub createLatestInRows()
    ...
    for each lib in m.filteredLatest
        if not isStringEqual(lib.collectionType, "boxsets") and not isStringEqual(lib.collectionType, "livetv") and not isStringEqual(lib.json.CollectionType, "Program")
            sectionName = (bslib_toString(tr("Recently Added in")) + " " + bslib_toString(lib.name))
            imagesize = ([
                464
                261
            ])
            ...
            try
                ...
                m.global.sceneManager.callFunc("standardDialog", "Error creating Recently Added rows", {
                    data: [
                        "<p>" + ("Error Message: " + bslib_toString(e.message)) + "</p>"
                    ]
                })
            end try
        end if
...
'//# sourceMappingURL=./HomeRows.brs.map