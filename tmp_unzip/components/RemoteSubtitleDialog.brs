...
sub setPalette()
    ...
    dlgPalette.colors = {
        ...
        DialogSecondaryTextColor: "#6F7FB7"
        DialogSecondaryItemColor: chainLookupReturn(m.global.session, "user.settings.colorDialogBorderLine", "#1F8DBA")
...
sub onButtonSelected()
    if not m.canProcessButton then
        return
    end if
...
sub onItemFocused()
    ...
    if isValid(m.focusedTextLine)
        m.focusedTextLine.color = chainLookupReturn(m.global.session, "user.settings.colorDialogText", "#ffffff")
    end if
    if isValid(m.focusedNameLine)
        m.focusedNameLine.color = chainLookupReturn(m.global.session, "user.settings.colorDialogText", "#ffffff")
        m.focusedNameLine.repeatCount = 0
    end if
    if isValid(m.focusedCommentLine)
        m.focusedCommentLine.color = chainLookupReturn(m.global.session, "user.settings.colorDialogText", "#ffffff")
        m.focusedCommentLine.repeatCount = 0
    end if
    focusedNameLine = focusedChild.focusedChild.findNode(("name" + bslib_toString(focusedChild.id)))
    if isValid(focusedNameLine)
        focusedNameLine.repeatCount = -1
        focusedNameLine.color = chainLookupReturn(m.global.session, "user.settings.colorDialogSelectedText", "#ffffff")
        m.focusedNameLine = focusedNameLine
    end if
    focusedCommentLine = focusedChild.focusedChild.findNode(("comment" + bslib_toString(focusedChild.id)))
    if isValid(focusedCommentLine)
        focusedCommentLine.repeatCount = -1
        focusedCommentLine.color = chainLookupReturn(m.global.session, "user.settings.colorDialogSelectedText", "#ffffff")
        m.focusedCommentLine = focusedCommentLine
    end if
    focusedTextLine = focusedChild.focusedChild.findNode(("text" + bslib_toString(focusedChild.id)))
    if isValid(focusedTextLine)
        focusedTextLine.color = chainLookupReturn(m.global.session, "user.settings.colorDialogSelectedText", "#ffffff")
        m.focusedTextLine = focusedTextLine
    end if
    moveScrollBar()
...
sub moveScrollBar()
    ...
    if m.scrollBarColumn.count() = 0
        ...
        m.scrollBarThumb[0].node.blendColor = "#1F8DBA"
...
sub onScrollBarFocus()
    ...
    m.scrollBarThumb[0].node.blendColor = "#6F7FB7"
end sub
...
sub onContentDataChanged()
    ...
    for each item in m.top.contentData.data
        ...
        textLine = cardGroup.CreateChild("ScrollingText")
        textLine.color = chainLookupReturn(m.global.session, "user.settings.colorDialogText", "#ffffff")
        textLine.id = ("name" + bslib_toString(i))
        textLine.maxWidth = "690"
        textLine.text = item.track.description
        textLine.repeatCount = 0
        textLine = cardGroup.CreateChild("Text")
        textLine.id = ("text" + bslib_toString(i))
        textLine.color = chainLookupReturn(m.global.session, "user.settings.colorDialogText", "#ffffff")
        textLine.Width = "690"
        textLine.text = (bslib_toString(tr("Format")) + ": " + bslib_toString(item.format) + "   " + bslib_toString(item.downloadcount) + " " + bslib_toString(tr("downloads")))
        textLine.font.size = 30
        if item.comment <> ""
            textLine = cardGroup.CreateChild("ScrollingText")
            textLine.color = chainLookupReturn(m.global.session, "user.settings.colorDialogText", "#ffffff")
            textLine.id = ("comment" + bslib_toString(i))
            textLine.maxWidth = "690"
...
'//# sourceMappingURL=./RemoteSubtitleDialog.brs.map