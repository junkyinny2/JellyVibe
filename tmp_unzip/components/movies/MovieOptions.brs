...
sub init()
    ...
    m.optionMenu.focusBitmapBlendColor = "#7B2FBE"
    ...
    m.videoNames = []
    m.audioNames = []
    m.top.findNode("videoMenu").focusBitmapBlendColor = "#7B2FBE"
    m.top.findNode("videoMenu").focusedColor = "#ffffff"
    m.top.findNode("audioMenu").focusBitmapBlendColor = "#7B2FBE"
    m.top.findNode("audioMenu").focusedColor = "#ffffff"
    m.top.findNode("subtitleMenu").focusBitmapBlendColor = "#7B2FBE"
...
function onKeyEvent(key as string, press as boolean) as boolean
    ...
    if key = "down" or (key = "OK" and m.top.findNode("buttons").hasFocus())
        traceStep("MOVIE_TRACE", "MovieOptions.onKeyEvent enter menu key=" + key)
        ...
        traceStep("MOVIE_TRACE", "MovieOptions.onKeyEvent enter menu done item=" + stri(m.selectedItem).trim())
        return true
    end if
    if key = "OK"
    ...
    if key = "back" or key = "up"
        if m.menus[m.selectedItem].isInFocusChain()
            ...
            traceStep("MOVIE_TRACE", "MovieOptions.onKeyEvent return to tab buttons key=" + key)
            return true
        end if
    end if
    if key = "options"
...
sub showSubtitleMenu()
    m.buttons.focusedIndex = 0
    buttonFocusChanged()
end sub
