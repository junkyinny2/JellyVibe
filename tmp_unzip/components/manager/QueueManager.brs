...
' Play items in queue
sub playQueue()
    traceStep("PLAY_TRACE", "QueueManager.playQueue start count=" + stri(getCount()).trim() + " position=" + stri(getPosition()).trim())
    m.isPlaying = true
    nextItem = getCurrentItem()
    if not isValid(nextItem)
        traceStep("PLAY_TRACE", "QueueManager.playQueue abort: nextItem invalid")
        return
    end if
    nextItemMediaType = getItemType(nextItem)
    if nextItemMediaType = ""
        traceStep("PLAY_TRACE", "QueueManager.playQueue abort: mediaType empty")
        return
    end if
    nextItemIdText = chainLookupReturn(nextItem, "id", "<none>")
    traceStep("PLAY_TRACE", "QueueManager.playQueue nextItem id=" + nextItemIdText + " type=" + nextItemMediaType)
    ...
    if nextItemMediaType = "audio"
        traceStep("PLAY_TRACE", "QueueManager.playQueue launching AudioPlayerView for audio")
        CreateAudioPlayerView()
        return
    end if
    if nextItemMediaType = "audiobook"
        traceStep("PLAY_TRACE", "QueueManager.playQueue launching AudioPlayerView for audiobook")
        CreateAudioPlayerView()
        return
    end if
    if nextItemMediaType = "musicvideo"
        traceStep("PLAY_TRACE", "QueueManager.playQueue launching VideoPlayerView for musicvideo")
        CreateVideoPlayerView()
        return
    end if
    if nextItemMediaType = "video"
        traceStep("PLAY_TRACE", "QueueManager.playQueue launching VideoPlayerView for video")
        CreateVideoPlayerView()
        return
    end if
    if nextItemMediaType = "tvchannel"
        traceStep("PLAY_TRACE", "QueueManager.playQueue launching VideoPlayerView for tvchannel")
        CreateVideoPlayerView()
        return
    end if
    if nextItemMediaType = "movie"
        traceStep("PLAY_TRACE", "QueueManager.playQueue launching VideoPlayerView for movie")
        CreateVideoPlayerView()
        return
    end if
    if nextItemMediaType = "episode"
        traceStep("PLAY_TRACE", "QueueManager.playQueue launching VideoPlayerView for episode")
        CreateVideoPlayerView()
        return
    end if
    if nextItemMediaType = "recording"
        traceStep("PLAY_TRACE", "QueueManager.playQueue launching VideoPlayerView for recording")
        CreateVideoPlayerView()
        return
    end if
