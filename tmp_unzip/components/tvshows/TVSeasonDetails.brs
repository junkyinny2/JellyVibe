...
sub init()
    ...
    m.settingsButton.highlightBackground = chainLookupReturn(m.global.session, "user.settings.colorCursor", "#7B2FBE")
    ...
    m.rows.focusBitmapBlendColor = chainLookupReturn(m.global.session, "user.settings.colorCursor", "#7B2FBE")
...
sub OnScreenShown()
    ...
    stopLoadingSpinner() ' Safety: ensure spinner is cleared if doneLoading never fired
    ...
    if isValid(m.top.lastFocus) and not m.top.lastFocus.isSameNode(m.overview)
...
sub enterEpisodeListFullScreen()
    if m.toplevel.translation[1] <> 40 then
        return
    end if
...
function onKeyEvent(key as string, press as boolean) as boolean
    ...
    if m.buttonGrp.isInFocusChain()
        ...
        if isStringEqual(key, "down")
            selectedBtn = m.buttonGrp.getChild(m.top.selectedButtonIndex)
            if isValid(selectedBtn) and selectedBtn.escape = "down"
                selectedBtn.escape = ""
                selectedBtn.focus = false
                m.rows.setFocus(true)
