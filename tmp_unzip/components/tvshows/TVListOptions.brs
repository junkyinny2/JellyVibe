...
sub init()
    ...
    m.videoNames = []
    m.audioNames = []
    m.top.findNode("videoMenu").focusBitmapBlendColor = chainLookupReturn(m.global.session, "user.settings.colorCursor", "#7B2FBE")
    m.top.findNode("videoMenu").focusedColor = "#ffffff"
    m.top.findNode("audioMenu").focusBitmapBlendColor = chainLookupReturn(m.global.session, "user.settings.colorCursor", "#7B2FBE")
...
'//# sourceMappingURL=./TVListOptions.brs.map