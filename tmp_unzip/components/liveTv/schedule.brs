...
sub init()
    ...
    m.scheduleGrid.focusBitmapBlendColor = chainLookupReturn(m.global.session, "user.settings.colorCursor", "#7B2FBE")
    m.scheduleGrid.programTitleFocusedColor = "#ffffff"
    m.scheduleGrid.timeLabelColor = "#aaaaaa"
...
sub loadChannelList()
    if not isStringEqual(m.state, "RUN") then
        return
    end if
    startLoadingSpinner()
    m.LoadChannelsTask.observeField("channels", "onChannelListLoaded")
...
sub onFilterSet()
    ...
    if not isValid(m.top.filter) then
        return
    end if
    if isStringEqual(m.LoadChannelsTask.filter, m.top.filter) then
        return
    end if
    m.scheduleGrid.jumpToChannel = 0
    if m.LoadChannelsTask.state = "RUN" then
...
sub loadScheduleData()
    ...
    if m.channelScheduleLoadedCount >= 1 then
        startingIndex = m.channelScheduleLoadedCount - 1
    else
        startingIndex = 0
    end if
...
sub onProgramDetailsLoaded()
    ...
    if m.LoadProgramDetailsTask.programDetails = invalid then
        return
    end if
...
sub focusProgramDetails(setFocused)
    ...
    h = h + 160 + 80
    if setFocused = true
        m.gridMoveAnimationPosition.keyValue = [
            [
                0
                600
            ]
...
sub onWatchChannelSelected()
    if m.detailsPane.watchSelectedChannel = false then
        return
    end if
...
sub onRecordChannelSelected()
    if m.detailsPane.recordSelectedChannel = false then
        return
    end if
    ' Set focus back to grid before showing channel, to ensure grid has focus when we return
    focusProgramDetails(false)
...
sub onRecordSeriesChannelSelected()
    if m.detailsPane.recordSeriesSelectedChannel = false then
        return
    end if
    ' Set focus back to grid before showing channel, to ensure grid has focus when we return
    focusProgramDetails(false)
...
'//# sourceMappingURL=./schedule.brs.map