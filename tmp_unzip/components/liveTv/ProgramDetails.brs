...
sub init()
    ...
    m.image.observeField("loadStatus", "onPosterLoadStatusChanged")
...
sub programUpdated()
    ...
    if prog.seasonNumber > 0 and prog.episodeNumber > 0
        ...
        if prog.episodeTitle <> "" then
            m.episodeNumber.text = m.episodeNumber.text + " -"
        end if ' Add a Dash if showing Episode Number and Title
    ...
    m.image.uri = ImageURL(prog.id, "Primary", {
        "maxHeight": 440
        "maxWidth": 295
    })
...
sub onPosterLoadStatusChanged()
    if isStringEqual(m.image.loadStatus, "ready")
        if m.image.loadWidth > 0
            m.image.unobserveField("loadStatus")
        end if
    end if
    if isStringEqual(m.image.loadStatus, "failed")
        if m.image.loadWidth = 0
            if isChainValid(m.top.programDetails, "PosterURL")
                m.image.uri = m.top.programDetails.PosterURL
            end if
        end if
    end if
end sub
...
sub focusChanged()
    if m.top.hasFocus = true
        m.overview.maxLines = m.maxDetailLines
        m.viewChannelFocusAnimationOpacity.keyValue = [
            0
            1
        ]
        m.recordFocusAnimationOpacity.keyValue = [
            0
            1
        ]
        m.recordSeriesFocusAnimationOpacity.keyValue = [
            0
            1
        ]
        ...
        m.viewChannelButtonBackground.blendColor = chainLookupReturn(m.global.session, "user.settings.colorCursor", "#7B2FBE")
...
function onKeyEvent(key as string, press as boolean) as boolean
    ...
    if m.userCanRecord = true
        if key = "right" and m.viewChannelButton.hasFocus()
            ...
            group = m.global.sceneManager.callFunc("getActiveScene")
            group.lastFocus = m.recordButton
            m.viewChannelOutline.visible = false
            m.recordOutline.visible = true
            m.viewChannelButtonBackground.blendColor = "#000000"
            m.recordButtonBackground.blendColor = chainLookupReturn(m.global.session, "user.settings.colorCursor", "#7B2FBE")
            m.recordSeriesButtonBackground.blendColor = "#000000"
            return true
        else if key = "right" and m.recordButton.hasFocus()
            m.recordSeriesButton.setFocus(true)
            group = m.global.sceneManager.callFunc("getActiveScene")
            group.lastFocus = m.recordSeriesButton
            m.recordOutline.visible = false
            m.recordSeriesOutline.visible = true
            m.viewChannelButtonBackground.blendColor = "#000000"
            m.recordButtonBackground.blendColor = "#000000"
            m.recordSeriesButtonBackground.blendColor = chainLookupReturn(m.global.session, "user.settings.colorCursor", "#7B2FBE")
            return true
...
'//# sourceMappingURL=./ProgramDetails.brs.map