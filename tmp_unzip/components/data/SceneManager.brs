...
sub optionClosed()
    print "[SCENEMGR] optionClosed called on node " + m.top.id
    ...
    print "[SCENEMGR] Setting dataReturned = true on node " + m.top.id
...
' Return button the user selected
sub optionSelected()
    print "[SCENEMGR] optionSelected called on node " + m.top.id + ", buttonSelected=" + stri(m.scene.dialog.buttonSelected).trim()
    m.userselection = true
    m.top.returnData = {
        id: m.scene.dialog.id
        itemID: m.itemID
