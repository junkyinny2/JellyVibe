...
sub setFields()
    ...
    ' Set full path as subtitle if available
    if isValid(json.path) and json.path <> ""
        m.top.shortdescriptionline1 = json.path
    end if
    ...
    if not isValid(m.top.json.Type) or m.top.json.Type <> "CollectionFolder"
        setPoster()
    end if
end sub
...
sub setPoster()
    ...
    else if isValid(m.top.json.Type) and m.top.json.Type = "Studio" and isValid(m.top.json.ParentThumbImageTag)
        imgParams = {
            "maxHeight": 440
            "maxWidth": 295
    ...
    else if isValid(m.top.json.ImageTags) and m.top.json.ImageTags.Primary <> invalid
        imgParams = {
            "maxHeight": 440
            "maxWidth": 295
    ...
    ' Fallback to folder icon if no poster found - ensures proper aspect ratio display
    if not isValidAndNotEmpty(m.top.posterURL)
        m.top.posterURL = m.top.iconUrl
    end if
end sub
