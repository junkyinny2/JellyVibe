...
sub init()
    traceStep("PLAY_TRACE", "VideoPlayerView.init start")
    ...
    currentItemIdText = chainLookupReturn(currentItem, "id", "<none>")
    currentItemTypeText = chainLookupReturn(currentItem, "type", "<none>")
    traceStep("PLAY_TRACE", "VideoPlayerView.init currentItem id=" + currentItemIdText + " type=" + currentItemTypeText)
    ...
    traceStep("PLAY_TRACE", "VideoPlayerView.init LoadVideoContentTask started")
    ...
    traceStep("PLAY_TRACE", "VideoPlayerView.init PreloadTrickplayImagesTask prepared for id=" + currentItemIdText)
    ...
    m.chapterMenu.focusBitmapBlendColor = chainLookupReturn(m.global.session, "user.settings.colorCursor", "#7B2FBE")
    ...
    m.subtitleSyncPlus100.focusBackground = "#7B2FBE"
    ...
    m.subtitleSyncPlus500.focusBackground = "#7B2FBE"
    ...
    m.subtitleSyncMinus100.focusBackground = "#7B2FBE"
    ...
    m.subtitleSyncMinus500.focusBackground = "#7B2FBE"
    ...
    m.subtitleSyncReset.focusBackground = "#7B2FBE"
    ...
    m.skipSegmentButton.focusBackground = chainLookupReturn(m.global.session, "user.settings.colorCursor", "#7B2FBE")
    ...
    m.top.retrievingBar.filledBarBlendColor = "#6F7FB7"
    m.top.bufferingBar.filledBarBlendColor = "#6F7FB7"
    m.top.trickPlayBar.filledBarBlendColor = "#6F7FB7"
    traceStep("PLAY_TRACE", "VideoPlayerView.init end")
end sub
...
' showCompatibilityWarningDialog: Presents a modal warning before direct-play
' starts so the user can choose to continue or return to the previous screen.
'
' @param {object} warnings - array of warning strings from checkRokuVideoCompatibility
sub showCompatibilityWarningDialog(warnings as object)
    m.compatibilityWarningContinue = false
    dialog = createObject("roSGNode", "Dialog")
    dialog.title = tr("Playback Compatibility Warning")
    dialog.buttons = [
        tr("Continue Anyway")
        tr("Go Back")
    ]
    dialog.message = warnings.join(chr(10))
    dialog.observeField("buttonSelected", "onCompatibilityWarningButtonSelected")
    dialog.observeField("wasClosed", "onCompatibilityWarningDialogClosed")
    m.top.getScene().dialog = dialog
end sub

' Track which button was pressed so wasClosed knows what action to take.
sub onCompatibilityWarningButtonSelected(msg)
    sourceNode = msg.getRoSGNode()
    m.compatibilityWarningContinue = (sourceNode.buttonSelected = 0)
    sourceNode.close = true
end sub

' After the dialog closes, either start playback or exit the player.
sub onCompatibilityWarningDialogClosed(msg)
    sourceNode = msg.getRoSGNode()
    sourceNode.unobserveField("buttonSelected")
    sourceNode.unobserveField("wasClosed")
    if m.compatibilityWarningContinue
        ' Arm the automatic transcode fallback: if direct play fails before any
        ' progress is reported, VideoPlayerView will set retryWithTranscoding=true
        ' and MainEventHandlers will silently retry with transcoding enabled.
        m.top.transcodeAvailable = true
        m.top.control = "PLAY"
    else
        m.global.sceneManager.callFunc("popScene")
    end if
...
sub onVideoContentLoaded()
    ...
    m.compatibilityWarnings = videoContent[0].compatibilityWarnings
    ' Transfer transcodeAvailable so the auto-fallback path works if direct play fails
    if isValid(videoContent[0].transcodeAvailable) and videoContent[0].transcodeAvailable
        m.top.transcodeAvailable = true
    end if
    ...
    m.top.setFocus(true)
    ' If the video is being direct-played and the source stream has known Roku
    ' hardware limitations, show a non-fatal warning dialog before starting playback.
    ' Transcoded content is already re-encoded by the server so no warning is needed.
    '
    ' Emby-Mode: skip the pre-play warning and attempt playback immediately.
    ' If playback fails, the error dialog will include the compatibility warnings.
    if not m.isTranscoded and isValidAndNotEmpty(m.compatibilityWarnings) and not CompatibilityBypass_IsEnabled()
        showCompatibilityWarningDialog(m.compatibilityWarnings)
    else
        m.top.control = "PLAY"
    end if
end sub
    ...
    if isStringEqual(m.top.state, "buffering") and m.bufferCheckTimer <> invalid
        ...
        if not m.playReported and m.top.transcodeAvailable
            ...
            if m.playbackRetries < 1
                ...
                m.global.sceneManager.callFunc("userMessage", tr("Live TV Playback Failed"), tr("An error was encountered while playing this live stream."))
                ' Stop playback and exit player
                m.top.control = "STOP"
                m.top.backPressed = true
                return
            ...
            if m.playbackRetries < 1
                ...
                print "Player error encountered - retry attempt " + m.playbackRetries.ToStr() + " of " + 1.ToStr()
                ...
                print "Playback failed after " + 1.ToStr() + " retry attempts"
                ' If we have compatibility info, show it alongside the generic error
                ' so the user understands why this file couldn't play.
                errorMsg = tr("Error During Playback")
                ' Include raw player error if available
                if isValid(m.top.errorMsg) and m.top.errorMsg <> ""
                    errorMsg += ": " + m.top.errorMsg
                end if
                if isValid(m.top.errorCode) and m.top.errorCode <> 0
                    errorMsg += " (Code " + m.top.errorCode.ToStr() + ")"
                end if
                if isValidAndNotEmpty(m.compatibilityWarnings)
                    errorMsg += chr(10) + chr(10) + "Compatibility Warnings:" + chr(10) + m.compatibilityWarnings.join(chr(10))
                end if
                ' Use internal dialog system which handles focus and population correctly
                showPlaybackErrorDialog(errorMsg)
                ' Stop playback and dismiss spinner
                m.top.control = "STOP"
    ...
    if m.top.bufferingStatus <> invalid
        ...
        if m.top.bufferingStatus["percentage"] > m.bufferPercentage
            ...
            if m.playbackRetries < 1
                m.playbackRetries = m.playbackRetries + 1
                m.lastBufferError = currentTime
                print "Buffering stalled - retry attempt " + m.playbackRetries.ToStr() + " of " + 1.ToStr()
                ...
                print "Buffering failed after " + 1.ToStr() + " retry attempts"
                showPlaybackErrorDialog(tr("There was an error retrieving the data for this item from the server."))
                ' Stop playback and exit player
                m.top.control = "STOP"
                m.top.backPressed = true
