...
sub init()
    ...
    m.progressBarBackground.color = "0x00000077"
    m.progressBar.color = "#6F7FB7"
    optionButtons = m.optionControls.getChildren(-1, 0)
    for each button in optionButtons
        button.background = "#101010"
        button.focusBackground = chainLookupReturn(m.global.session, "user.settings.colorCursor", "#7B2FBE")
    end for
...
sub onSeasonNumberChanged()
    ...
    itemSeason = m.top.findNode("itemSeason")
    itemSeason.font.size = 32
    itemSeason.text = ("S" + bslib_toString(m.top.seasonNumber))
    ' Move the option controls down to give room for season number
    if not m.optionControlsMoved
        moveOptionControls(0, 50)
        m.optionControlsMoved = true
...
sub onEpisodeNumberChanged()
    ...
    itemEpisode.text = ("E" + bslib_toString(m.top.episodeNumber))
...
sub onEpisodeNumberEndChanged()
    ...
    itemEpisodeEnd.text = ("-" + bslib_toString(m.top.episodeNumberEnd))
    ' Move the option controls down to give room for episode number
    if not m.optionControlsMoved
        moveOptionControls(0, 50)
        m.optionControlsMoved = true
...
sub moveOptionControls(horizontalPixels as integer, verticalPixels as integer)
    m.optionControls.translation = ("[" + bslib_toString(m.optionControls.translation[0] + horizontalPixels) + ", " + bslib_toString(m.optionControls.translation[1] + verticalPixels) + "]")
end sub
...
sub onLogoLoadStatusChanged(event)
    if LCase(event.GetData()) = "ready"
        ...
        m.videoInfo.translation = ("[" + bslib_toString(m.videoInfo.translation[0] + logoImage.bitmapWidth + 30) + ", " + bslib_toString(m.videoInfo.translation[1]) + "]")
        m.itemTitle.maxWidth = m.itemTitle.maxWidth - (logoImage.bitmapWidth + 30)
...
sub checkDisplaySiblingItem(selectedButtonID)
    if not isValidAndNotEmpty(selectedButtonID) then
        selectedButtonID = ""
    end if
    ...
    if key = "right"
        ...
        for i = m.focusedButtonIndex + 1 to buttonGroup.getChildCount()
            ...
            if isValid(focusedButton.focus)
                ...
                if m.videoControls.isInFocusChain() then
