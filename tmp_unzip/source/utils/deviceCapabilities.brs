...
function getDeviceCapabilities(videoCodec = "" as string) as object
    deviceProfile = {
        "PlayableMediaTypes": [
            "Audio"
            "Video"
...
function getDeviceProfile(videoCodec = "" as string) as object
    ...
    ' Apply "Triple Lie" overrides if enabled
    CompatibilityBypass_ApplyDeviceCaps(profile)
    return profile
end function
...
function GetDirectPlayProfiles() as object
    ...
    directPlayProfiles = []
    if not isStringEqual("playNormally", chainLookupReturn(globalUserSettings, "`playback.media.forceTranscode`", "playNormally")) then
        return directPlayProfiles
    end if
    if not isStringEqual(m.global.queueManager.callFunc("getForceTranscode"), "playNormally") then
        return directPlayProfiles
    end if
...
function getTranscodingProfiles(videoCodec = "" as string) as object
    globalUserSettings = m.global.session.user.settings
    transcodingProfiles = []
    di = CreateObject("roDeviceInfo")
    ...
    mp4AudioCodecs = "AC3"
    if isStringEqual(videoCodec, "h264") then
        mp4VideoCodecs = ""
    else
        mp4VideoCodecs = "h264"
    end if
...
function getCodecProfiles(videoCodec = "" as string) as object
    globalUserSettings = m.global.session.user.settings
    codecProfiles = []
    profileSupport = {
        "h264": {}
    ...
    ' AVC / h264 / MPEG4 AVC
    ' In Emby-Mode, advertise maximum support so the server picks Direct Play
    ' for almost all H.264 content. AV1 and VP9 are never faked since they are
    ' truly unsupported at the hardware level.
    embyMode = CompatibilityBypass_IsEnabled()
    if embyMode
        h264Profiles = [
            "baseline"
            "main"
    ...
    for each profile in h264Profiles
        for each level in h264Levels
            if embyMode
                ' In Emby-Mode, skip the CanDecodeVideo call and add directly
                if not isStringEqual(videoCodec, "h264")
    ...
    if not isStringEqual(videoCodec, "")
    ...
    ' Apply Level override if bypass is on
    h264LevelString = CompatibilityBypass_GetForcedLevel("h264", h264LevelString)
    h264ProfileArray = {
        "Type": "Video"
        "Codec": "h264"
        "Conditions": [
    ...
    if not globalUserSettings["playback.media.directPlayAnamorphic"]
    ...
    if not globalUserSettings["playback.compatibility.disablehevc"] and di.CanDecodeVideo({
        ...
        ' Apply Level override if bypass is on
        hevcLevelString = CompatibilityBypass_GetForcedLevel("hevc", hevcLevelString)
        hevcProfileArray = {
            "Type": "Video"
            "Codec": "hevc"
            "Conditions": [
                {
                    "Condition": "EqualsAny"
                    "Property": "VideoProfile"
                    "Value": hevcAssProfiles.Keys().join("|")
                    "IsRequired": false
                }
                {
                    "Condition": "EqualsAny"
    ...
    if di.CanDecodeVideo({
        ...
        if not globalUserSettings["playback.media.directPlayAnamorphic"]
            vp9ProfileArray.Conditions.push({
                "Condition": "NotEquals"
                "Property": "IsAnamorphic"
                "Value": "true"
                "IsRequired": false
...
'//# sourceMappingURL=./deviceCapabilities.brs.map