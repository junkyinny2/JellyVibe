...
function currentTimestamp() as string
    ...
    dt.ToLocalTime()
    year = leftPad(stri(dt.getYear()).trim(), "0", 4)
    month = leftPad(stri(dt.getMonth()).trim(), "0", 2)
    day = leftPad(stri(dt.getDayOfMonth()).trim(), "0", 2)
    hour = leftPad(stri(dt.getHours()).trim(), "0", 2)
    minute = leftPad(stri(dt.getMinutes()).trim(), "0", 2)
    second = leftPad(stri(dt.getSeconds()).trim(), "0", 2)
    return (bslib_toString(year) + "-" + bslib_toString(month) + "-" + bslib_toString(day) + " " + bslib_toString(hour) + ":" + bslib_toString(minute) + ":" + bslib_toString(second))
end function

sub traceStep(tag as string, message = "" as string)
    if isValidAndNotEmpty(message)
        print ("[" + bslib_toString(currentTimestamp()) + "] " + bslib_toString(tag) + " | " + bslib_toString(message))
    else
        print ("[" + bslib_toString(currentTimestamp()) + "] " + bslib_toString(tag))
    end if
end sub

function debugBool(value as dynamic) as string
    if isValid(value) and value
        return "true"
    end if
    return "false"
end function
...
' Whether library views may show/update a backdrop image for the focused item (user setting).
function libraryFocusBackdropEnabled() as boolean
    val = chainLookupReturn(m.global.session, "user.settings.itemgrid.focusBackdrop", true)
    if val = invalid then
        return true
    end if
    if type(val) = "roString" or type(val) = "String"
        return not isStringEqual(LCase(val.trim()), "false")
    end if
    return val = true
end function
...
sub startLoadingSpinner(disableRemote = true as boolean)
    ...
    if not isValid(scene) and isValid(m.scene)
        scene = m.scene
    end if
    if not isValid(scene)
        traceStep("MOVIE_TRACE", "Spinner.start scene invalid")
        return
    end if
    wasLoading = scene.isLoading
    wasDisableRemote = scene.disableRemote
    if not scene.isLoading
        scene.disableRemote = disableRemote
        scene.isLoading = true
    end if
    traceStep("MOVIE_TRACE", "Spinner.start requestedDisableRemote=" + debugBool(disableRemote) + " wasLoading=" + debugBool(wasLoading) + " wasDisableRemote=" + debugBool(wasDisableRemote) + " nowLoading=" + debugBool(scene.isLoading) + " nowDisableRemote=" + debugBool(scene.disableRemote))
end sub
...
sub stopLoadingSpinner()
    ...
    if not isValid(scene)
        traceStep("MOVIE_TRACE", "Spinner.stop scene invalid")
        return
    end if
    wasLoading = scene.isLoading
    wasDisableRemote = scene.disableRemote
    if scene.isLoading
        scene.disableRemote = false
        scene.isLoading = false
    end if
    traceStep("MOVIE_TRACE", "Spinner.stop wasLoading=" + debugBool(wasLoading) + " wasDisableRemote=" + debugBool(wasDisableRemote) + " nowLoading=" + debugBool(scene.isLoading) + " nowDisableRemote=" + debugBool(scene.disableRemote))
end sub
