...
function ItemMetaData(id as string, includePosterImage = true as boolean)
    url = Substitute("Items/{0}", id)
    resp = APIRequest(url, {
        userid: m.global.session.user.id
        fields: "Chapters,Trickplay,People,TagItems,UserData"
    })
    data = getJson(resp)
    ...
    if data.type = "Movie" or data.type = "MusicVideo"
        ...
        if includePosterImage and tmp.image = invalid
            tmp.image = PosterImage(data.id, {
                "MaxWidth": 500
                "MaxHeight": 500
            })
        ...
        ' Song's parent poster image is no good, try using the song's poster image
        if includePosterImage and not isValid(tmp.image)
            tmp.image = PosterImage(data.ParentId, {
                "MaxWidth": 500
                "MaxHeight": 500
            })
