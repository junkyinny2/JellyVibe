...
function buildURL(path as string, params = {} as object) as dynamic
    ...
    full_url = (bslib_toString(parsed_url.proto) + "://" + bslib_toString(parsed_url.host) + bslib_toString((function(__bsCondition, parsed_url)
            if __bsCondition then
                return ""
            else
                return (":" + bslib_toString(parsed_url.port))
            end if
        end function)(parsed_url.port = "", parsed_url)) + bslib_toString(parsed_url.path))
    ...
    if realPath.startswith("/") then
        full_url += realPath
    else
        full_url += ("/" + bslib_toString(realPath))
    end if
...
function deleteVoid(req) as boolean
    print "[DELETE] Starting deleteVoid"
    req.setMessagePort(CreateObject("roMessagePort"))
    req.AddHeader("Content-Type", "application/json")
    ...
    resp = wait(30000, req.GetMessagePort())
    if type(resp) <> "roUrlEvent"
        print "[DELETE] Invalid response type: " + type(resp)
        return false
    end if
    responseCode = resp.GetResponseCode()
    print "[DELETE] Response code: " + stri(responseCode)
    ' HTTP 204 No Content is the expected success response for DELETE
    if responseCode = 204 or responseCode = 200
        print "[DELETE] Success!"
        return true
    end if
    print "[DELETE] Failed with response code: " + stri(responseCode)
    return false
end function
...
'//# sourceMappingURL=./baserequest.brs.map